        ...
        for product in state.order_depths:
            ...
            if product == 'KELP':
                ...
                if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
                    ...
                    # We are now improving the prices by 1
                    if curr_ask - curr_bid > 4:
                        if imbal >  50:
                            orders.append(Order(product, list(order_depth.buy_orders.items())[0][0], 1))
                        elif imbal < -50:
                            orders.append(Order(product, list(order_depth.sell_orders.items())[0][0], -1))
                        
                        orders.append(Order(product, list(order_depth.buy_orders.items())[0][0]+1, 2))
                        orders.append(Order(product, list(order_depth.sell_orders.items())[0][0]-1, -2))
