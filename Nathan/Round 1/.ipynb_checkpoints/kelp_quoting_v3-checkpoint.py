            ...
            if product == "KELP":
                ...
                if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
                    if len(self.kelp_df) >= 2:
                        ...
                        ask_pca = (-0.67802679 * (current_row.ask_volume_1 if current_row.ask_volume_1 is not None else 0) + 
           0.73468115 * (current_row.ask_volume_2 if current_row.ask_volume_2 is not None else 0) + 
           0.02287503 * (current_row.ask_volume_3 if current_row.ask_volume_3 is not None else 0))

                        bid_pca = (-0.69827525 * (current_row.bid_volume_1 if current_row.bid_volume_1 is not None else 0) + 
           0.71532596 * (current_row.bid_volume_2 if current_row.bid_volume_2 is not None else 0) + 
           0.02684134 * (current_row.bid_volume_3 if current_row.bid_volume_3 is not None else 0))

                        lag_1_bidvol_return_interaction = bid_pca * current_log_return
                        lag_1_askvol_return_interaction = ask_pca * current_log_return
                        future_log_return_prediction =-0.0000035249 + 0.0000070160 * ask_pca + -0.0000069054 * bid_pca -0.2087831028 * current_log_return -0.0064021782* lag_1_askvol_return_interaction + -0.0049996728 * lag_1_bidvol_return_interaction
                        ...
                        print(f"current_midprice: {current_midprice}")
                        
                        theo = future_price_prediction - kelp_position * 0.05 #0.05 is our retreat, can be changed.
                        #theo = current_midprice - kelp_position * 0.05 #This is testing the strategy without a future_price prediction.
                        #theo = current_midprice #This is testing the strategy without a retreat
                        #theo = future_price_prediction #This is testing the strategy without a retreat
                        print(theo)
                        my_bid = int(np.floor(theo))
                        my_ask = int(np.ceil(theo))
                        bid_edge = theo - my_bid
                        ask_edge = my_ask - theo #We measure edge as a positive number.
                        edge0 = 0.2
                        if bid_edge > edge0:
                            bid_volume = int(np.floor((bid_edge - edge0) / 0.1))
                        else:
                            bid_volume = 0
                        if ask_edge > edge0:
                            ask_volume = -int(np.floor((ask_edge - edge0) / 0.1))
                        else:
                            ask_volume = 0 
                        orders.append(Order(product, my_bid, bid_volume))
                        orders.append(Order(product, my_ask, ask_volume))
                        print(f"my_bid: {my_bid}, my_ask: {my_ask}, bid_volume: {bid_volume}, ask_volume: {ask_volume}")
