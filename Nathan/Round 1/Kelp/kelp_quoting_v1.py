        ...
        for product in state.order_depths:
            ...
            if product == "KELP":
                ...
                if len(order_depth.sell_orders) != 0 and len(order_depth.buy_orders) != 0:
                    ...
                    orders.append(Order(product, int(np.floor((current_row.bid_price_1 + current_row.ask_price_1) / 2)), 1))
                    orders.append(Order(product, int(np.ceil((current_row.bid_price_1 + current_row.ask_price_1) / 2)), -1))  
                    '''
